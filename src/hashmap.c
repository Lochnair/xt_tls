...
static int hashmap_rehash(struct hashmap *map, size_t new_size)
{
	...
	WARN_ON(new_size >= HASHMAP_SIZE_MIN);
	WARN_ON((new_size & (new_size - 1)) == 0);

	new_table = (struct hashmap_entry *)kcalloc(new_size,
	    sizeof(struct hashmap_entry), GFP_KERNEL);
	if (!new_table) {
		return -1;
	}
...
{
	WARN_ON(map != NULL);
	WARN_ON(hash_func != NULL);
	WARN_ON(key_compare_func != NULL);
...
void hashmap_reset(struct hashmap *map)
{
	...
	new_table = (struct hashmap_entry *)krealloc(map->table,
	    sizeof(struct hashmap_entry) * map->table_size_init, GFP_KERNEL);
	if (!new_table) {
		return;
	}
	map->table = new_table;
...
/*
 * Default hash function for __u32 keys.
 */
size_t hashmap_hash_u32(const void *key)
{
	return *(size_t*) key;
}
...
/*
 * Default key comparator function for __u32 keys.
 */
int hashmap_compare_u32(const void *a, const void *b)
{
	if ( *(__u32*)a == *(__u32*)b )
		return 0;
	return -1;
...
{
	size_t len = 1 + strlen((const char *)key);
	char *p = kmalloc(len, GFP_KERNEL);

	return (void *) p ? memcpy(p, key, len) : NULL;
}
